	...
	bool task7Ok =
	    ...
	    containsAllNeedles(evaluateShader,
	                       {"float placementThreshold;",
	                        ...
	                        "(pixelSource.sourceFlags & SURFEL_PT_SOURCE_FLAG_VALID) != 0u",
	                        ...
	                        "allocateSurfel(pixel, position, normal, radius, resolveSurfelRadiance(position, normal), loadPixelMaterialKey(pixel));"}) &&
	    containsAllNeedles(passesCpp,
	                       {"placementThreshold",
	                        "removalThreshold",
	                        "surfelTargetArea",
	...
	bool sourceInstanceTablesOk =
	    ...
	    containsAllNeedles(updateShader,
	                       {"uint sourceTransformCount;",
	                        ...
	                        "(source.sourceFlags & SURFEL_PT_SOURCE_FLAG_VALID) == 0u",
	                        ...
	                        "(transform.flags & SURFEL_PT_SOURCE_FLAG_VALID) == 0u",
	                        "mul(transform.objectToWorld, float4(source.sourceObjectPosition, 1.0))",
	                        "mul(transpose((float3x3)transform.worldToObject), objectNormal)",
